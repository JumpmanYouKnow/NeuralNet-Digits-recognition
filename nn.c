...
// #if VALIDATION
// #define VALIDATION_PROPORTION 0.33
// #endif
 ...
 */
#define N_EXAMPLES 5646

#if VALIDATION
#define MIN_INCREASE 5
#endif
...
#if VALIDATION
void    splitValidation( int n_input, int n_output, dd_type *all_data,
...
main( int argc, char *argv[] )
{
	...
	// #if VALIDATION
	// 	splitValidation( nn.n_input, nn.n_output,
	// 	&training_data, &validation_data, &training_data2);
	// 	trainNetwork( log_file, &nn, &training_data2);
	// #else
...
//      */
//     training_data->n = VALIDATION_PROPORTION * all_data->n;
...
trainNetwork( FILE *log_file, nn_type *nn, dd_type *data )
{
    ...
    void    adjustWeights();
    #if VALIDATION
    	int last_errors, cur_errors;
    	int n_increase = 0;
        int min_errors = data->n;
    #endif
	...
	for( p = 0; p < data->n; p++ ) {
		...
		adjustWeights( nn, data->x[p], data->y[p] );
                    ...
                    #if VALIDATION
        for( p = 0; p < data->n; p++ ) {
            determineOutput( nn, test_data.x[p] );
                for( j = 1; j <= nn->n_output; j++ ) {
                    if( fabs( nn->o[j] - test_data.y[p][j] ) > ACCEPT_TEST ) {
                        cur_errors++;
                        break;
                    }
                }
        ...
        fprintf( log_file, "Number of classification errors: %d (/%d)\n",
			errors, data->n );
        #if VALIDATION
        fprintf( log_file, "Number of validation errors: %d (/%d)\n",
			cur_errors, test_data.n );
        #endif
	fflush( log_file );

	#if VALIDATION
        if (cur_errors < min_errors) {
            min_errors = cur_errors;
        } else if (cur_errors - min_errors >= 300) {
            break;
        }
		...
		if (n_increase >= MIN_INCREASE) {
			break;               // increasing errors in validation set
		}
	#endif
    /*
     *  Stopping criteria.
